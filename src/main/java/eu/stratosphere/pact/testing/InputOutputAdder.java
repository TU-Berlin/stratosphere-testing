...
class InputOutputAdder implements Visitor<Contract> {
		...
		for (int index = 0; index < inputs.size(); index++)
			if (inputs.get(index).isEmpty())
				inputs.get(index).add(
					GenericTestPlan.createDefaultSource(String.format("%s-input%d", contract.getName(), index)));
	...
	private Contract replaceWithDefaultOutput(final Contract contract) {
		final FileDataSink defaultSink = GenericTestPlan.createDefaultSink(contract.getName() + "-output");
